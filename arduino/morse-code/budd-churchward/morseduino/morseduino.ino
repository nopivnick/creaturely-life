...
#define VDATE "2016.03.07"          //Sketch Version Date
...
void setup() {
  ...
  if (!digitalRead(LEFT_BUTTON)) speaker = false; //Hold left button down to mute speaker
...
void resetDefaults() {
  ...
  // only write to EEPROM is value has changed
...
void truncateOverFlow() {
  ...
  if (lastSpace == 0) {
    return; // Don't do this if there was no space in the last line
  }
